...
class UitarsRFModel():
    def __init__(self, 
                 ...
                 model_name="bytedance-research/UI-TARS-72B-DPO",
                 api_key="empty"):
        """
    ...
    def ground(self, instruction, image):
        ...
        full_prompt = (
            f'Output the most relevant point in the image corresponding to the instruction "{instruction}" with grounding.'
        )
    ...
    def judge_inference(self, instruction, image, point, debug=False, task_id=None):
        """
        ...
        highlighted_image = plot_points_on_image(
            pil_image,
            [point],
            colors=[(255, 105, 180, 128)],  # Magenta
            markers=['star'],
            sizes=[20]
        )
        ...
        #is_correct = ("YES" in response.upper().split() or "CORRECT" in response.upper().split()) and not ("NO" in response.upper().split())
        is_correct = ("YES" in response.upper() or "CORRECT" in response.upper() or "正确" in response or "精准" in response) and not ("NO" in response.upper() or "不正确" in response or "不精准" in response)
    ...
    def region_focus(self, instruction, image, debug=False, task_id=None, temperature=0, top_p=1.0):
        """
    ...
    def next_action_regionfocus(self, instruction, zoomed_img_bytes, left, top, zoom_x, zoom_y, 
                              ...
                              task_id=None, index=None, temperature=0, top_p=1.0):
        """
        ...
        action_prompt = (
            ...
            f'the instruction: "{instruction}". Return only the x,y coordinates (integers) '
        ...
        if click_point:
            ...
            x_upsampled = round(x_upsampled*zoomed_width/1000)
            y_upsampled = round(y_upsampled*zoomed_height/1000)
    ...
    def next_action_regionfocus_aggregation(self, instruction, image, points, debug=False, task_id=None):
        """
        ...
        aggregated_image = plot_points_on_image(
            ...
            markers=['star' for _ in range(len(points))],
            sizes=[15 for _ in range(len(points))],
            labels=labels
        )
        ...
        selection_prompt = (
            ...
            f'First explain your reasoning for each point, then provide your final answer in this format: '
    ...
    def ground_with_regionfocus(self, instruction, image, debug=False, task_id=None):
        ...
        initial_result = self.ground(instruction, image)
        ...
        if initial_result["point"]:
            ...
            if debug:
                ...
                print(f"Judgment response: {judge_response[:200]}...")
        ...
        # We'll try up to 3 different temperature settings to get diverse points
        ...
        if not zoomed_results:
            ...
            return initial_result if initial_result["point"] else {"point": region_points[0], "bbox": None}
