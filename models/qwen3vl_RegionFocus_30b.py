...
class Qwen3VLModel():
    ...
    def ground_allow_negative(self, instruction, image):
        """
        Ground with support for negative samples (element not found).
        ...
        Returns:
            dict: Result dictionary with result type (positive/negative) and point if applicable
        """
        # For now, use the same grounding approach
        # In a production system, you might want to add explicit negative detection
        result_dict, _, _ = self.ground(instruction, image)
        return result_dict
